"""ID-consistency test for the Book 5 rewrite page.

NOTE ON PROVENANCE: this script was regenerated after publication from the
test_ids.py pattern used for Books 3, 4 and 6, because no standalone Book 5
test script was saved from the original run. It checks the published page as
it exists in this repository. It is not the script from the original
verification run.
...
Asserts (exit nonzero on ANY failure):
  ...
  6. GRAPHS config count matches container count (6 figures)
"""
import os, re, sys

HTML = "/home/hatch/workspace/r-theory-rewrite/book5/index.html"
BASE = "/home/hatch/workspace/r-theory-rewrite/book5"
fails = []
...
# --- 6. figure count ---
if len(configs) != 6:
    fail(f"expected 6 figure configs, found {len(configs)}")

if fails:
    print(f"\n{len(fails)} FAILURE(S) — see above")
    sys.exit(1)
print("\nALL ID TESTS PASSED: 6 configs, 6 containers, 6 fallbacks, 6 PNGs, no dupes")
